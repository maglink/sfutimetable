...
AUDIT_PAT = "(\d{3}\w|\d{3}|\d{1,3}-\d{1,3})"
...
def process_file(file_name):
	...
	edge_x, edge_y, hasntWeeks = find_edge(sheet)

	for specialty, group, x, y in read_specialties(sheet, edge_x, edge_y, hasntWeeks):
		for lesson in read_group(sheet, x, y, hasntWeeks):
...
def read_specialties(sheet, x, y, hasntWeeks):
	ncols = sheet.ncols
	has_next_group = True
	...
	specialty = sheet.cell(y, x).value
	if not specialty:
		raise Exception("Speciality is not defined")

	groupName = sheet.cell(y + 1, x).value
	if not groupName:
		groupName = sheet.cell(y + 2, x).value
		y += 1
	if not groupName:
...
def read_group(sheet, x, y, hasntWeeks):
	...
	while 1:
		...
		secondWeekNotExist = False
		if not hasntWeeks:
			if ypos+3 >= nrows:
				secondWeekNotExist = True
			else:
				if sheet.cell(ypos+3, 0).value != '' and sheet.cell(ypos+3, 0).value != day:
					secondWeekNotExist = True
				if sheet.cell(ypos+3, 1).value != '' and sheet.cell(ypos+3, 1).value != time:
					secondWeekNotExist = True
		else:
			secondWeekNotExist = True
...
def process_lesson(lesson_raw):
	...
	lesson_data = {"lesson_type": lesson_type, "subject": subject, "teacher_list": teacher_list,
